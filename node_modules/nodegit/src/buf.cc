...
GitBuf::GitBuf(git_buf *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitBuf::JSNewFunction)
{
  ...
  GitBuf* object = new GitBuf(static_cast<git_buf *>(
...
Local<v8::Value> GitBuf::New(const git_buf *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitBuf::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitBuf::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitBuf::GetNonSelfFreeingConstructedCount)
{
...
int GitBuf::SelfFreeingInstanceCount;
int GitBuf::NonSelfFreeingConstructedCount;
