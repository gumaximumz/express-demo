...
GitNote::GitNote(git_note *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
void GitNote::InitializeComponent(Local<v8::Object> target)
{
...
NAN_METHOD(GitNote::JSNewFunction)
{
  ...
  if (info.Length() == 0 || !info[0]->IsExternal()) {
    return Nan::ThrowError("A new GitNote cannot be instantiated.");
  }

  GitNote* object = new GitNote(static_cast<git_note *>(
...
Local<v8::Value> GitNote::New(const git_note *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitNote::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitNote::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitNote::GetNonSelfFreeingConstructedCount)
{
...
// end convert_from_v8 block
  ...
  Nan::Callback *callback = new Nan::Callback(Local<Function>::Cast(info[7]));
...
void GitNote::RemoveWorker::HandleOKCallback()
{
  if (baton->error_code == GIT_OK) {
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method remove has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        Local<v8::Value> argv[1] = {
          err
        };
...
Nan::Persistent<Function> GitNote::constructor_template;

int GitNote::SelfFreeingInstanceCount;
int GitNote::NonSelfFreeingConstructedCount;
