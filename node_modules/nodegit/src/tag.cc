...
GitTag::GitTag(git_tag *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitTag::JSNewFunction)
{
  ...
  GitTag* object = new GitTag(static_cast<git_tag *>(
...
Local<v8::Value> GitTag::New(const git_tag *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitTag::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitTag::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitTag::GetNonSelfFreeingConstructedCount)
{
...
int GitTag::SelfFreeingInstanceCount;
int GitTag::NonSelfFreeingConstructedCount;
