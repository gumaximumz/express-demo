...
GitCredUsername::GitCredUsername(git_cred_username *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitCredUsername::JSNewFunction)
{
  ...
  GitCredUsername* object = new GitCredUsername(static_cast<git_cred_username *>(
...
Local<v8::Value> GitCredUsername::New(const git_cred_username *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitCredUsername::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitCredUsername::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitCredUsername::GetNonSelfFreeingConstructedCount)
{
...
int GitCredUsername::SelfFreeingInstanceCount;
int GitCredUsername::NonSelfFreeingConstructedCount;
