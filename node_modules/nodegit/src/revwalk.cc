...
GitRevwalk::GitRevwalk(git_revwalk *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitRevwalk::JSNewFunction)
{
  ...
  GitRevwalk* object = new GitRevwalk(static_cast<git_revwalk *>(
...
Local<v8::Value> GitRevwalk::New(const git_revwalk *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitRevwalk::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitRevwalk::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitRevwalk::GetNonSelfFreeingConstructedCount)
{
...
int GitRevwalk::SelfFreeingInstanceCount;
int GitRevwalk::NonSelfFreeingConstructedCount;
