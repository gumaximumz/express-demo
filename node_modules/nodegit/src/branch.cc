...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(/*asyncAction: */false        ,    from_repository
                                           ,    from_branch_name
                                           ,    from_commit
                         );
...
void GitBranch::SetUpstreamWorker::HandleOKCallback()
{
  if (baton->error_code == GIT_OK) {
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method setUpstream has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        Local<v8::Value> argv[1] = {
          err
        };
...
// end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method upstream has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        Local<v8::Value> argv[1] = {
          err
        };
...
int GitBranch::SelfFreeingInstanceCount;
int GitBranch::NonSelfFreeingConstructedCount;
