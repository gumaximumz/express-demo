...
GitRefs::GitRefs(git_reference *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitRefs::JSNewFunction)
{
  ...
  GitRefs* object = new GitRefs(static_cast<git_reference *>(
...
Local<v8::Value> GitRefs::New(const git_reference *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitRefs::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitRefs::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitRefs::GetNonSelfFreeingConstructedCount)
{
...
// start convert_from_v8 block
  const git_reference * from_ref2;
  from_ref2 = Nan::ObjectWrap::Unwrap<GitRefs>(info[0]->ToObject())->GetValue();
// end convert_from_v8 block

  giterr_clear();

  {
    LockMaster lockMaster(/*asyncAction: */false        ,    Nan::ObjectWrap::Unwrap<GitRefs>(info.This())->GetValue()
                                           ,    from_ref2
                         );
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(/*asyncAction: */false        ,    from_refname
                         );
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(/*asyncAction: */false        ,    from_buffer_out
                                           ,    from_name
                         );
...
// start convert_to_v8 block
    ...
    if (baton->new_ref != NULL) {
      ...
      to = GitRefs::New(baton->new_ref, true  );
    } else {
      to = Nan::Null();
    }
...
// start convert_to_v8 block
    ...
    if (result != NULL) {
      // GitOid result
      to = GitOid::New(result, false , info.This() );
    } else {
      to = Nan::Null();
    }
...
int GitRefs::SelfFreeingInstanceCount;
int GitRefs::NonSelfFreeingConstructedCount;
