...
// start convert_from_v8 block
  ...
  from_rules = (const char *) malloc(rules.length() + 1);
  // copy the characters from the nodejs string into our C-string (used instead of strdup or strcpy because nulls in
  // the middle of strings are valid coming from nodejs):
  memcpy((void *)from_rules, *rules, rules.length());
  // ensure the final byte of our new string is null, extra casts added to ensure compatibility with various C types
  // used in the nodejs binding generation:
  memset((void *)(((char *)from_rules) + rules.length()), 0, 1);
// end convert_from_v8 block
...
NAN_METHOD(GitIgnore::PathIsIgnored)
{
  ...
  PathIsIgnoredBaton* baton = new PathIsIgnoredBaton;
...
// end convert_from_v8 block
  ...
  PathIsIgnoredWorker *worker = new PathIsIgnoredWorker(baton, callback);
...
void GitIgnore::PathIsIgnoredWorker::Execute()
{
  giterr_clear();

  {
    LockMaster lockMaster(/*asyncAction: */true        ,baton->ignored
                                           ,baton->repo
                                           ,baton->path
                         );

    int result = git_ignore_path_is_ignored(
                   baton->ignored,baton->repo,baton->path    );

  }
}

void GitIgnore::PathIsIgnoredWorker::HandleOKCallback()
{
  if (baton->error_code == GIT_OK) {
    Local<v8::Value> to;
// start convert_to_v8 block
    to = Nan::New<Number>(* baton->ignored);
    // end convert_to_v8 block
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method pathIsIgnored has thrown an error.")->ToObject();
...
int GitIgnore::SelfFreeingInstanceCount;
int GitIgnore::NonSelfFreeingConstructedCount;
