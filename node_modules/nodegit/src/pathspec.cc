...
GitPathspec::GitPathspec(git_pathspec *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitPathspec::JSNewFunction)
{
  ...
  GitPathspec* object = new GitPathspec(static_cast<git_pathspec *>(
...
Local<v8::Value> GitPathspec::New(const git_pathspec *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitPathspec::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitPathspec::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitPathspec::GetNonSelfFreeingConstructedCount)
{
...
int GitPathspec::SelfFreeingInstanceCount;
int GitPathspec::NonSelfFreeingConstructedCount;
