...
GitCred::GitCred(git_cred *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitCred::JSNewFunction)
{
  ...
  GitCred* object = new GitCred(static_cast<git_cred *>(
...
Local<v8::Value> GitCred::New(const git_cred *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitCred::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitCred::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitCred::GetNonSelfFreeingConstructedCount)
{
...
int GitCred::SelfFreeingInstanceCount;
int GitCred::NonSelfFreeingConstructedCount;
