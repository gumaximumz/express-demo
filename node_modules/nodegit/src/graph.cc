...
void GitGraph::DescendantOfWorker::HandleOKCallback()
{
  if (baton->error_code >= GIT_OK) {
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method descendantOf has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        Local<v8::Value> argv[1] = {
          err
        };
...
int GitGraph::SelfFreeingInstanceCount;
int GitGraph::NonSelfFreeingConstructedCount;
