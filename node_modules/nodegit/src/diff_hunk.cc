...
GitDiffHunk::GitDiffHunk(git_diff_hunk *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitDiffHunk::JSNewFunction)
{
  ...
  GitDiffHunk* object = new GitDiffHunk(static_cast<git_diff_hunk *>(
...
Local<v8::Value> GitDiffHunk::New(const git_diff_hunk *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitDiffHunk::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitDiffHunk::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitDiffHunk::GetNonSelfFreeingConstructedCount)
{
...
int GitDiffHunk::SelfFreeingInstanceCount;
int GitDiffHunk::NonSelfFreeingConstructedCount;
