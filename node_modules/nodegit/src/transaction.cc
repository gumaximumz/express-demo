...
GitTransaction::GitTransaction(git_transaction *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
void GitTransaction::InitializeComponent(Local<v8::Object> target)
{
...
NAN_METHOD(GitTransaction::JSNewFunction)
{

  if (info.Length() == 0 || !info[0]->IsExternal()) {
    return Nan::ThrowError("A new GitTransaction cannot be instantiated.");
  }

  GitTransaction* object = new GitTransaction(static_cast<git_transaction *>(
...
Local<v8::Value> GitTransaction::New(const git_transaction *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitTransaction::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitTransaction::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitTransaction::GetNonSelfFreeingConstructedCount)
{
  info.GetReturnValue().Set(NonSelfFreeingConstructedCount);
}

git_transaction *GitTransaction::GetValue()
{
  return this->raw;
}

void GitTransaction::ClearValue()
{
  this->raw = NULL;
}

Nan::Persistent<Function> GitTransaction::constructor_template;

int GitTransaction::SelfFreeingInstanceCount;
int GitTransaction::NonSelfFreeingConstructedCount;
