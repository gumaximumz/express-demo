...
GitFilterList::GitFilterList(git_filter_list *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitFilterList::JSNewFunction)
{
  ...
  GitFilterList* object = new GitFilterList(static_cast<git_filter_list *>(
...
Local<v8::Value> GitFilterList::New(const git_filter_list *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitFilterList::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitFilterList::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitFilterList::GetNonSelfFreeingConstructedCount)
{
...
int GitFilterList::SelfFreeingInstanceCount;
int GitFilterList::NonSelfFreeingConstructedCount;
