...
GitCvarMap::GitCvarMap(git_cvar_map *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitCvarMap::JSNewFunction)
{
  ...
  GitCvarMap* object = new GitCvarMap(static_cast<git_cvar_map *>(
...
Local<v8::Value> GitCvarMap::New(const git_cvar_map *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitCvarMap::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitCvarMap::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitCvarMap::GetNonSelfFreeingConstructedCount)
{
...
int GitCvarMap::SelfFreeingInstanceCount;
int GitCvarMap::NonSelfFreeingConstructedCount;
