...
void GitReset::InitializeComponent(Local<v8::Object> target)
{
...
void GitReset::ResetWorker::HandleOKCallback()
{
  if (baton->error_code == GIT_OK) {
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method reset has thrown an error.")->ToObject();
...
void GitReset::DefaultWorker::HandleOKCallback()
{
  if (baton->error_code == GIT_OK) {
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method default has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        Local<v8::Value> argv[1] = {
          err
        };
...
// end convert_from_v8 block
// start convert_from_v8 block
  git_annotated_commit * from_commit;
...
Nan::Persistent<Function> GitReset::constructor_template;

int GitReset::SelfFreeingInstanceCount;
int GitReset::NonSelfFreeingConstructedCount;
