...
GitRebase::GitRebase(git_rebase *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitRebase::JSNewFunction)
{
  ...
  GitRebase* object = new GitRebase(static_cast<git_rebase *>(
...
Local<v8::Value> GitRebase::New(const git_rebase *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitRebase::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitRebase::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitRebase::GetNonSelfFreeingConstructedCount)
{
...
int GitRebase::SelfFreeingInstanceCount;
int GitRebase::NonSelfFreeingConstructedCount;
