...
GitTreeEntry::GitTreeEntry(git_tree_entry *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
    ...
    git_tree_entry_dup(&this->raw, raw);
    selfFreeing = true;
  } else {
    this->raw = raw;
  }
...
NAN_METHOD(GitTreeEntry::JSNewFunction)
{
  ...
  GitTreeEntry* object = new GitTreeEntry(static_cast<git_tree_entry *>(
...
Local<v8::Value> GitTreeEntry::New(const git_tree_entry *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitTreeEntry::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitTreeEntry::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitTreeEntry::GetNonSelfFreeingConstructedCount)
{
...
int GitTreeEntry::SelfFreeingInstanceCount;
int GitTreeEntry::NonSelfFreeingConstructedCount;
