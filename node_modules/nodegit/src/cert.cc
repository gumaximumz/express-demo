...
GitCert::GitCert(git_cert *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitCert::JSNewFunction)
{
  ...
  GitCert* object = new GitCert(static_cast<git_cert *>(
...
Local<v8::Value> GitCert::New(const git_cert *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitCert::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitCert::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitCert::GetNonSelfFreeingConstructedCount)
{
...
int GitCert::SelfFreeingInstanceCount;
int GitCert::NonSelfFreeingConstructedCount;
