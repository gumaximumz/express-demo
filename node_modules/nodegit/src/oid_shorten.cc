...
GitOidShorten::GitOidShorten(git_oid_shorten *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitOidShorten::JSNewFunction)
{
  ...
  GitOidShorten* object = new GitOidShorten(static_cast<git_oid_shorten *>(
...
Local<v8::Value> GitOidShorten::New(const git_oid_shorten *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitOidShorten::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitOidShorten::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitOidShorten::GetNonSelfFreeingConstructedCount)
{
...
int GitOidShorten::SelfFreeingInstanceCount;
int GitOidShorten::NonSelfFreeingConstructedCount;
