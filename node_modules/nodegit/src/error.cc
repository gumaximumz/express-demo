...
GitError::GitError(git_error *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitError::JSNewFunction)
{
  ...
  GitError* object = new GitError(static_cast<git_error *>(
...
Local<v8::Value> GitError::New(const git_error *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitError::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitError::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitError::GetNonSelfFreeingConstructedCount)
{
...
int GitError::SelfFreeingInstanceCount;
int GitError::NonSelfFreeingConstructedCount;
