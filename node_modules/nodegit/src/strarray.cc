...
GitStrarray::GitStrarray(git_strarray *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitStrarray::JSNewFunction)
{
  ...
  GitStrarray* object = new GitStrarray(static_cast<git_strarray *>(
...
Local<v8::Value> GitStrarray::New(const git_strarray *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitStrarray::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitStrarray::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitStrarray::GetNonSelfFreeingConstructedCount)
{
...
int GitStrarray::SelfFreeingInstanceCount;
int GitStrarray::NonSelfFreeingConstructedCount;
