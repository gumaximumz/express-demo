...
GitSignature::GitSignature(git_signature *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
    ...
    git_signature_dup(&this->raw, raw);
    selfFreeing = true;
  } else {
    this->raw = raw;
  }
...
NAN_METHOD(GitSignature::JSNewFunction)
{
  ...
  GitSignature* object = new GitSignature(static_cast<git_signature *>(
...
Local<v8::Value> GitSignature::New(const git_signature *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitSignature::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitSignature::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitSignature::GetNonSelfFreeingConstructedCount)
{
...
int GitSignature::SelfFreeingInstanceCount;
int GitSignature::NonSelfFreeingConstructedCount;
