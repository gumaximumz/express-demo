...
GitPathspecMatchList::GitPathspecMatchList(git_pathspec_match_list *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitPathspecMatchList::JSNewFunction)
{
  ...
  GitPathspecMatchList* object = new GitPathspecMatchList(static_cast<git_pathspec_match_list *>(
...
Local<v8::Value> GitPathspecMatchList::New(const git_pathspec_match_list *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitPathspecMatchList::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitPathspecMatchList::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitPathspecMatchList::GetNonSelfFreeingConstructedCount)
{
...
int GitPathspecMatchList::SelfFreeingInstanceCount;
int GitPathspecMatchList::NonSelfFreeingConstructedCount;
