...
GitIndex::GitIndex(git_index *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitIndex::JSNewFunction)
{
  ...
  GitIndex* object = new GitIndex(static_cast<git_index *>(
...
Local<v8::Value> GitIndex::New(const git_index *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitIndex::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitIndex::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitIndex::GetNonSelfFreeingConstructedCount)
{
...
int GitIndex::SelfFreeingInstanceCount;
int GitIndex::NonSelfFreeingConstructedCount;
