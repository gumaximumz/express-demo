...
GitWritestream::GitWritestream(git_writestream *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitWritestream::JSNewFunction)
{
  ...
  GitWritestream* object = new GitWritestream(static_cast<git_writestream *>(
...
Local<v8::Value> GitWritestream::New(const git_writestream *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitWritestream::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitWritestream::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitWritestream::GetNonSelfFreeingConstructedCount)
{
...
int GitWritestream::SelfFreeingInstanceCount;
int GitWritestream::NonSelfFreeingConstructedCount;
