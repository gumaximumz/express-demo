...
GitRepository::GitRepository(git_repository *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitRepository::JSNewFunction)
{
  ...
  GitRepository* object = new GitRepository(static_cast<git_repository *>(
...
Local<v8::Value> GitRepository::New(const git_repository *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitRepository::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitRepository::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitRepository::GetNonSelfFreeingConstructedCount)
{
...
void GitRepository::MergeheadForeachWorker::HandleOKCallback()
{
  if (baton->error_code == GIT_OK) {
    ...
    if (baton->error) {
      ...
      if (!callbackFired) {
        Local<v8::Object> err = Nan::Error("Method mergeheadForeach has thrown an error.")->ToObject();
        err->Set(Nan::New("errno").ToLocalChecked(), Nan::New(baton->error_code));
        Local<v8::Value> argv[1] = {
          err
        };
...
{
  MergeheadForeach_CallbackBaton* baton = new MergeheadForeach_CallbackBaton();
  ...
  uv_async_init(uv_default_loop(), &baton->req, (uv_async_cb) MergeheadForeach_callback_async);
  {
    LockMaster::TemporaryUnlock temporaryUnlock;
...
void GitRepository::MergeheadForeach_callback_async(uv_async_t* req, int status)
{
  Nan::HandleScope scope;

  MergeheadForeach_CallbackBaton* baton = static_cast<MergeheadForeach_CallbackBaton*>(req->data);
  ...
  Nan::TryCatch tryCatch;
  Local<v8::Value> result = callback->Call(2, argv);
...
int GitRepository::SelfFreeingInstanceCount;
int GitRepository::NonSelfFreeingConstructedCount;
