...
GitBlameHunk::GitBlameHunk(git_blame_hunk *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitBlameHunk::JSNewFunction)
{
  ...
  GitBlameHunk* object = new GitBlameHunk(static_cast<git_blame_hunk *>(
...
Local<v8::Value> GitBlameHunk::New(const git_blame_hunk *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitBlameHunk::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitBlameHunk::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitBlameHunk::GetNonSelfFreeingConstructedCount)
{
...
// start convert_to_v8 block
  ...
  if (final_commit_id != NULL) {
    ...
    to = GitOid::New(final_commit_id, false  );
  } else {
    to = Nan::Null();
  }
...
// start convert_to_v8 block
  ...
  if (final_signature != NULL) {
    ...
    to = GitSignature::New(final_signature, false  );
  } else {
    to = Nan::Null();
  }
...
// start convert_to_v8 block
  ...
  if (orig_commit_id != NULL) {
    ...
    to = GitOid::New(orig_commit_id, false  );
  } else {
    to = Nan::Null();
  }
...
// start convert_to_v8 block
  ...
  if (orig_signature != NULL) {
    ...
    to = GitSignature::New(orig_signature, false  );
  } else {
    to = Nan::Null();
  }
...
int GitBlameHunk::SelfFreeingInstanceCount;
int GitBlameHunk::NonSelfFreeingConstructedCount;
