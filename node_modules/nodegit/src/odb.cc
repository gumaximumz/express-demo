...
GitOdb::GitOdb(git_odb *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitOdb::JSNewFunction)
{
  ...
  GitOdb* object = new GitOdb(static_cast<git_odb *>(
...
Local<v8::Value> GitOdb::New(const git_odb *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitOdb::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitOdb::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitOdb::GetNonSelfFreeingConstructedCount)
{
...
int GitOdb::SelfFreeingInstanceCount;
int GitOdb::NonSelfFreeingConstructedCount;
