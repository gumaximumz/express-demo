...
GitTime::GitTime(git_time *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitTime::JSNewFunction)
{
  ...
  GitTime* object = new GitTime(static_cast<git_time *>(
...
Local<v8::Value> GitTime::New(const git_time *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitTime::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitTime::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitTime::GetNonSelfFreeingConstructedCount)
{
...
int GitTime::SelfFreeingInstanceCount;
int GitTime::NonSelfFreeingConstructedCount;
