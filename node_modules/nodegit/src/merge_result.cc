...
GitMergeResult::GitMergeResult(git_merge_result *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitMergeResult::JSNewFunction)
{
  ...
  GitMergeResult* object = new GitMergeResult(static_cast<git_merge_result *>(
...
Local<v8::Value> GitMergeResult::New(const git_merge_result *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitMergeResult::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitMergeResult::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitMergeResult::GetNonSelfFreeingConstructedCount)
{
...
int GitMergeResult::SelfFreeingInstanceCount;
int GitMergeResult::NonSelfFreeingConstructedCount;
