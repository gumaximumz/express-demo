...
GitTreebuilder::GitTreebuilder(git_treebuilder *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitTreebuilder::JSNewFunction)
{
  ...
  GitTreebuilder* object = new GitTreebuilder(static_cast<git_treebuilder *>(
...
Local<v8::Value> GitTreebuilder::New(const git_treebuilder *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitTreebuilder::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitTreebuilder::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitTreebuilder::GetNonSelfFreeingConstructedCount)
{
...
int GitTreebuilder::SelfFreeingInstanceCount;
int GitTreebuilder::NonSelfFreeingConstructedCount;
