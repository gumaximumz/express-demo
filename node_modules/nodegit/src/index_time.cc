...
GitIndexTime::GitIndexTime(git_index_time *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitIndexTime::JSNewFunction)
{
  ...
  GitIndexTime* object = new GitIndexTime(static_cast<git_index_time *>(
...
Local<v8::Value> GitIndexTime::New(const git_index_time *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitIndexTime::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitIndexTime::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitIndexTime::GetNonSelfFreeingConstructedCount)
{
...
int GitIndexTime::SelfFreeingInstanceCount;
int GitIndexTime::NonSelfFreeingConstructedCount;
