...
GitRefdb::GitRefdb(git_refdb *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitRefdb::JSNewFunction)
{
  ...
  GitRefdb* object = new GitRefdb(static_cast<git_refdb *>(
...
Local<v8::Value> GitRefdb::New(const git_refdb *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitRefdb::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitRefdb::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitRefdb::GetNonSelfFreeingConstructedCount)
{
...
NAN_METHOD(GitRefdb::Compress)
{
  ...
  {
    LockMaster lockMaster(/*asyncAction: */false        ,    Nan::ObjectWrap::Unwrap<GitRefdb>(info.This())->GetValue()
                         );
...
int GitRefdb::SelfFreeingInstanceCount;
int GitRefdb::NonSelfFreeingConstructedCount;
