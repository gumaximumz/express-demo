...
GitDiff::GitDiff(git_diff *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitDiff::JSNewFunction)
{
  ...
  GitDiff* object = new GitDiff(static_cast<git_diff *>(
...
Local<v8::Value> GitDiff::New(const git_diff *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitDiff::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitDiff::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitDiff::GetNonSelfFreeingConstructedCount)
{
...
// end convert_from_v8 block
  ...
  {
    LockMaster lockMaster(/*asyncAction: */false        ,    Nan::ObjectWrap::Unwrap<GitDiff>(info.This())->GetValue()
                         );
...
// start convert_to_v8 block
    ...
    if (baton->diff != NULL) {
      // GitDiff baton->diff
      to = GitDiff::New(baton->diff, false  );
    } else {
      to = Nan::Null();
    }
...
int GitDiff::SelfFreeingInstanceCount;
int GitDiff::NonSelfFreeingConstructedCount;
