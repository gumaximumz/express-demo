...
GitTree::GitTree(git_tree *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
void GitTree::InitializeComponent(Local<v8::Object> target)
{
  ...
  Nan::SetMethod(tpl, "entryFree", EntryFree);
...
NAN_METHOD(GitTree::JSNewFunction)
{
  ...
  GitTree* object = new GitTree(static_cast<git_tree *>(
...
Local<v8::Value> GitTree::New(const git_tree *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitTree::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitTree::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitTree::GetNonSelfFreeingConstructedCount)
{
...
// start convert_to_v8 block
    ...
    if (result != NULL) {
      // GitTreeEntry result
      to = GitTreeEntry::New(result, false  );
    } else {
      to = Nan::Null();
    }
...
// start convert_to_v8 block
    ...
    if (baton->out != NULL) {
      ...
      to = GitTreeEntry::New(baton->out, false  );
    } else {
      to = Nan::Null();
    }
...
int GitTree::SelfFreeingInstanceCount;
int GitTree::NonSelfFreeingConstructedCount;
