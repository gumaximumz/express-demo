...
GitIndexConflictIterator::GitIndexConflictIterator(git_index_conflict_iterator *raw, bool selfFreeing, Local<v8::Object> owner)
{
  if (!owner.IsEmpty()) {
...
NAN_METHOD(GitIndexConflictIterator::JSNewFunction)
{
  ...
  GitIndexConflictIterator* object = new GitIndexConflictIterator(static_cast<git_index_conflict_iterator *>(
...
Local<v8::Value> GitIndexConflictIterator::New(const git_index_conflict_iterator *raw, bool selfFreeing, Local<v8::Object> owner)
{
  Nan::EscapableHandleScope scope;
  Local<v8::Value> argv[3] = { Nan::New<External>((void *)raw), Nan::New(selfFreeing), owner };
  return scope.Escape(
           Nan::NewInstance(
             Nan::New(GitIndexConflictIterator::constructor_template),
             owner.IsEmpty() ? 2 : 3, // passing an empty handle as part of the arguments causes a crash
             argv
           ).ToLocalChecked());
}

NAN_METHOD(GitIndexConflictIterator::GetSelfFreeingInstanceCount)
{
  info.GetReturnValue().Set(SelfFreeingInstanceCount);
}

NAN_METHOD(GitIndexConflictIterator::GetNonSelfFreeingConstructedCount)
{
...
int GitIndexConflictIterator::SelfFreeingInstanceCount;
int GitIndexConflictIterator::NonSelfFreeingConstructedCount;
